...
- **Entry detail screen** (`/entry/{slug}`): header, diagnostic banner, schema-driven attributes with a preserved-fields block, raw body with highlighted wikilinks, and the rail panel (backlinks, outgoing links, metadata). Components in `frontend/src/lib/components/entry/`.
